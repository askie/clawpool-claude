...
如果要两边文档完全不打架，建议统一成下面这句话：

- Claude Clawpool 渠道的审批、提问、配对、访问状态，当前已经完成“文本驱动的卡片化渲染闭环”；
- 但 Claude 插件侧尚未改造成“结构化卡片载荷下发”，目前仍依赖 `reply_source + 固定文本格式`。

这样说最贴近现状，也能把“前端已完成”和“插件未结构化”同时说清楚。
...
- 卡片提交后直接走现有 request store resolve 流程
- 保留现有 `/clawpool-approval` 和 `/clawpool-question` 文本命令兜底
...
如果产品目标变成“远端也要像 Claude 终端一样操作 `/命令`”，那就不是单纯做卡片渲染了，而是要单独设计：
