...
test("question command parser handles single and indexed answers", () => {
  const single = parseQuestionResponseCommand("/clawpool-claude-question req-1 use production");
  ...
  const mapped = parseQuestionResponseCommand("/clawpool-claude-question req-2 1=prod; 2=cn-hz");
...
test("question command parser rejects malformed input", () => {
  const invalid = parseQuestionResponseCommand("/clawpool-claude-question req-3 1=");
