...
test("approval command parser handles allow and deny", () => {
  const allow = parseApprovalDecisionCommand("/clawpool-approval req-1 allow");
  assert.equal(allow.matched, true);
  assert.equal(allow.ok, true);
  assert.equal(allow.request_id, "req-1");
  assert.deepEqual(allow.resolution, { type: "allow" });

  const deny = parseApprovalDecisionCommand("/clawpool-approval req-2 deny not safe");
...
test("approval command parser handles allow-rule and invalid input", () => {
  const withRule = parseApprovalDecisionCommand("/clawpool-approval req-3 allow-rule 2");
  ...
  const invalid = parseApprovalDecisionCommand("/clawpool-approval req-3 allow-rule nope");
