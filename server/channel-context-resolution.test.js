...
test("hook channel context resolver prefers stored session context", async () => {
  const dir = await mkdtemp(path.join(os.tmpdir(), "grix-claude-context-resolution-"));
...
test("hook channel context resolver allows transcript fallback for a single grix-claude chat", async () => {
  const dir = await mkdtemp(path.join(os.tmpdir(), "grix-claude-context-resolution-"));
  const transcriptPath = path.join(dir, "transcript.jsonl");
  const lines = [
    JSON.stringify({
      type: "user",
      message:
        '<channel source="grix-claude" chat_id="chat-9" event_id="evt-9" message_id="msg-9" sender_id="u-9">hello</channel>',
    }),
